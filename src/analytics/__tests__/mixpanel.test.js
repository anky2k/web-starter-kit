...
import * as asyncScriptLoader from '../async-script-loader';
...
describe('------ mixpanel analytics ------', () => {
  it('should call script inject function on init', () => {
    ...
    mixpanel.init();
    expect(asyncScriptLoader.inject).toHaveBeenCalledWith('', scriptSrc);
  });
  ...
  test('at max three retries should happen for a failed script', async () => {
    window.mixpanel = null;
    await mixpanel.retry();
    expect(asyncScriptLoader.inject).toHaveBeenCalledTimes(4);
  });
});
